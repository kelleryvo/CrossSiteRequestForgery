...
// CSRF TOKEN
var csurf = require('csurf'); // to prevent csrf attack
...
app.get('/change_password', csrfProtection, function(req, res){
  res.render('change_password', {errors: '', csrfToken: req.csrfToken()});
});

app.post('/change_password', urlencodedParser, csrfProtection, function(req, res){
  var sess = req.session
  ...
  if(req.body.password && req.body.password_repeat){
    ...
    res.render('change_password', {errors: '<div class="well">CSRF: Request to change password to ' + password + ' / ' + password_repeat + '</div>', csrfToken: req.csrfToken()});
  } else {
    //Not all Parameters Given / False
    res.render('change_password', {errors: '<span class="label label-default">Invalid login credentials!</span>', csrfToken: req.csrfToken()});
  }
});
